...
import { loginFail, failText, somethingWrong, registerFail } from "../../constants/errors"
...
function App() {
  ...
  React.useEffect(() => {
    if (localStorage.jwt) {
      mainApi.validation(localStorage.jwt)
      .then(
        (response) => {
          if (response) {
            setCurrentUser(response);
  ...
  function handleSearchSubmit(event) {
    if (movies.length > 0) {
      ...
      const searchedSavedMovies = handleFilter(savedMovies, event.searchKeywords);
  ...
  const handleDeleteMovie = (savedMovie) => {
    mainApi
      ...
      .then((res) => {
        ...
        setFilteredSavedMovies(filteredSavedMovies.filter((item) => item._id !== res._id));
      })
      .catch((err) => {
        console.log(err.status, err.statusText);
      })
  ...
  function resetValidation() {
    ...
    React.useEffect(() => { //
      ...
      mainApi
        ...
        .then((data) => {
          const searchedSavedMovies = handleFilter(data, values.searchKeywords);
          const searchedSavedMoviesWithDuration = handleDurationFilter(searchedSavedMovies);

          if (searchedSavedMovies.length > 0 || searchedSavedMoviesWithDuration > 0) {
            setIsMoviesLoadError(false);
          } else {
            setIsMoviesLoadError(404);
          }
